...
| 19 | `tools/bypass_waf/` (à créer) | Sessions FlareSolverr non nettoyées automatiquement : si `session_id` est fourni mais que le job échoue ou expire, la session reste ouverte dans le conteneur FlareSolverr (fuite mémoire progressive). Implémenter un appel `sessions.destroy` en bloc `finally` dans le runner, et un TTL de session côté caller (ex : Redis expire à 30 min). | **Step 3.5** (runner bypass_waf) | ⏳ Ouvert |
| 20 | `tools/bypass_waf/` (à créer) | Pas de retry automatique sur timeout de challenge : si FlareSolverr expire (`max_timeout` atteint), le runner remonte l'erreur sans tentative de relance avec un nouveau `session_id`. Ajouter une logique retry (max 2 tentatives, nouvelle session à chaque fois) pour absorber les timeouts transitoires. | **Step 3.5** (runner bypass_waf) | ⏳ Ouvert |
| 21 | `bridge/scheduler.py` | APScheduler `daily_factory_run` tourne dans le thread principal asyncio du bridge. Si le job dépasse 09:00 du lendemain (warm-up lent, proxy down), les jobs se chevauchent. Ajouter `max_instances=1` sur `add_job()` et une alerte si la durée dépasse 45 min. | **Step 4 (monitoring)** | ⏳ Ouvert |
| 22 | `tools/screenshot_runner/profile_manager.py` | `_save_meta()` fait `INSERT OR REPLACE` sur tous les profils en mémoire — opération O(n). Sur un parc de 100+ profils, chaque micro-session ré-écrit la table entière. Remplacer par des UPDATE individuels ciblés. | **Step 5 (perf)** | ⏳ Ouvert |
| 23 | `tools/scrapy_runner/project/spiders/ebay_browse.py` | **TD-23 — eBay Marketplace Insights API** : `itemCreationDate` utilisé comme `start_date` proxy dans Browse API. L'Insights API (`buy/marketplace_insights/v1_beta`) fournirait sold prices + sold dates réelles mais requiert un compte partenaire eBay. Évalué en Step 4.5 : `avg_sell_days` calculé sur les items avec end_date non-null (source WatchCount uniquement pour l'instant). Coverage ePID correct sur Browse API. Insights API non prioritaire avant trafic > 10k items/jour. | **Step 5+** (si partenariat eBay) | 🟡 Veille |
| 24 | `tools/stats/epid_calculator.py` | **TD-24 — avg_sell_days fiabilité sur petit sample** : `avg_sell_days` est calculé dès `sell_days_sample = 1`, ce qui peut être trompeur. Une règle `sample >= 3` éviterait les faux signals pour les ePIDs rares. Impact faible en Session 4 (peu d'items avec end_date). Réévaluer quand le volume de scraping WatchCount augmente. | **Step 5** (monitoring qualité données) | ⏳ Ouvert |
...
- **TD-19/20** — ouverts au Step 3.5, toujours en attente d'implémentation (cible S4/S5).
- **TD-23** — réévalué au Step 4.5. Marketplace Insights API non prioritaire : avg_sell_days implémenté via items scraped avec end_date (source WatchCount). Statut passé à Veille.
